...
class CommandLineProcessor(cmd.Cmd):
	...
	def do_gamemode(self, params):
		""" Command to change the game mode.
		"""
		try:
			if params == "":
				self.output.write(f"Current game mode: {self.app.gamemode}")
				return

			# change to game mode with the params as int if a number or as a string otherwise
			self.app.change_gamemode(int(params) if params.isdigit() else params)
			self.output.write(f'Game mode changed to {self.app.gamemode}')
			return
		except Exception as E:
